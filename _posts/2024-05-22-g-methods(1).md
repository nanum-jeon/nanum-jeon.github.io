...
<script type="text/javascript">
  function toggle_visibility(event) {
    ...
    // Find the closest .toggle-content to the clicked link
    var element = event.target.nextElementSibling;
    while(element && !element.classList.contains('toggle-content')) {
      element = element.nextElementSibling;
    }
    
    if (element && (element.style.display === 'none' || element.style.display === '')) {
      element.style.display = 'block';
      // Tell MathJax to typeset the newly visible content
...
<a href="#" onclick="toggle_visibility(event);">[Settings in the Paper]</a>
...
$$E(Y^{a_0, a_1}) =  \sum_{z_1} E(Y | A_1 = a_1, Z_1 = z_1, A_0 = a_0) P(Z_1 = z_1 | A_0 = a_0)$$
