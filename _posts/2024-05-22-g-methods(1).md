...
However, we often use simulation from parametric regression models to construct a  **g-formula** estimator. This approach is typically the most realistic when dealing with many covariates in our data. Now, we will discuss how to use parametric regression models to apply the g-formula.
...
y_model <- glm(y ~ a1 + z1 + a0 + a1*a0, data = data, family = gaussian())
```

Note we are using logistic models for $Z_1$ and $A_1$ as they are binary varaibles, and using linear model for $Y$. All these models have parametric assumptions on the relationships between the variables, which may actually bias our estimates, if these assumptions do not hold in our data generating processes.   
...
|1              |0              |          121.4875|
|0              |1              |          128.4610|
|natural course |natural course |          124.9701|
